...
object ConnectivityWatchdog {
     ...
     *
     * Двадцать секунд — с запасом больше самой долгой пробы (8 с) и короче интервала проверок,
     * так что «живым» туннель числится только пока байты идут прямо сейчас, а не «шли когда-то».
     */
    private const val TRAFFIC_ALIVE_WINDOW_MS = 20_000L
    private const val PROBE_TIMEOUT_MS = 8_000

    /**
     * Сколько неудачных проверок подряд нужно, чтобы вмешаться. Было 2 — при интервале в 15
     * секунд это давало «полминуты неурядиц — и рвём всё». Стало 3: связь у человека дороже
     * лишних пятнадцати секунд ожидания, а лечение здесь недешёвое — смена узла или
     * переподключение обрывают то, что сейчас качается.
     */
    private const val FAILURES_BEFORE_HEAL = 3
    ...
    /**
     * Когда через туннель последний раз шёл ВСТРЕЧНЫЙ трафик — то есть приходили байты.
     *
     * Это самое сильное доказательство, что туннель жив: пробу можно не дождаться по сотне
     * причин, а вот принятые байты подделать нечем. Нужен, чтобы не объявлять сбой во время
     * тяжёлой передачи: канал занят файлом, рукопожатие пробы не укладывается в свои 8 секунд,
     * и сторож раньше делал вывод «туннель мёртв» — рвал соединения и ронял ту самую передачу.
     * Получался порочный круг: чем активнее человек пользуется связью, тем вероятнее сторож её
     * сломает. Журнал 20.08 — 34 разрыва за два часа, файл так и не догрузился.
     */
    @Volatile
    ...
    private object TrafficHandler : CommandClient.Handler {
        override fun updateStatus(status: StatusMessage) {
            ...
            // Пришли байты — засекаем момент. Дальше это защитит от ложной тревоги: см.
            // lastTrafficAt и TRAFFIC_ALIVE_WINDOW_MS.
            if (receiving) lastTrafficAt = System.currentTimeMillis()
    ...
    private suspend fun checkOnce(trigger: String = "цикл") = checkMutex.withLock {
        ...
        //
        // ★★ И второе доказательство жизни — ВСТРЕЧНЫЙ ТРАФИК. Если только что приходили байты,
        // туннель работает, и неудачная проба этого не отменяет: она могла не уложиться в срок
        // именно потому, что канал занят полезной передачей. Раньше сторож в такой момент решал,
        // что туннель мёртв, и рвал соединения — то есть ронял ту самую передачу, из-за которой
        // и не успел. Чем активнее человек пользовался связью, тем вернее сторож её ломал.
        val trafficAlive = System.currentTimeMillis() - lastTrafficAt < TRAFFIC_ALIVE_WINDOW_MS
        if (domainOk || trafficAlive) {
            ...
            if (now - lastHeartbeatAt >= HEARTBEAT_INTERVAL_MS) {
                lastHeartbeatAt = now
                // Помечаем случай «проба не прошла, но байты идут»: при разборе журнала важно
                // видеть, что сбоя не объявили именно из-за живого трафика, а не потому что
                // проба была успешной.
                val why = if (!domainOk) " · проба не прошла, но трафик идёт" else ""
                appendLog("всё в порядке · ${context(trigger)}$why")
        ...
        when {
            ...
            // ★ Туннель ЖИВ, сыплется что-то одно (обычно DNS). Начинаем с самого дешёвого —
            // смены узла. Раньше первой же ступенью шло переподключение: полная перезагрузка
            // ядра, 20–30 секунд немоты и обрыв всего, что качается. Слишком дорогая плата за
            // спотыкнувшийся резолв, тем более что смена узла теперь бесшовна — существующие
            // соединения она не рвёт (interrupt_exist_connections снят в подписке).
            healAttempts == 0 && autoNodes.isNotEmpty() -> {
                appendLog(str(R.string.watchdog_log_switch_server, what))
                switchToFastestServer()
            }
