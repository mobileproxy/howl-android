...
object AppEventLog {
    ...
    /**
     * ★ 2 МБ, а не 512 КБ. Прежнего размера хватало на трое-четверо суток спокойной работы, но
     * в аварию он вымывался за часы: ночь 14–15.08 дала около 560 переключений и столько же
     * строк с замерами, и события двух предыдущих суток ушли безвозвратно — остались только
     * строки ядра из своего файла. Разбирать аварию по журналу, который эта же авария и стёрла,
     * невозможно. Файлов два (текущий и предыдущий), так что на диске это до 4 МБ — для
     * приложения, весящего 30 МБ, приемлемая цена за возможность понять, что случилось.
     */
